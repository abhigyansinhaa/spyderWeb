  ...
  return (
    <section id="certifications" className="section-flat py-20 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        >
          <span className="text-indigo-400 font-mono text-base mr-2">05.</span>
          Certifications
        </motion.h2>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {CERTS.map((cert, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="group relative rounded-xl border border-white/8 card-gradient p-6 hover:border-indigo-500/20 hover:shadow-[0_4px_24px_-4px_rgba(99,102,241,0.1)] transition-all duration-300"
            >
              {/* Top glow on hover */}
              <div className="pointer-events-none absolute inset-x-0 -top-px h-px bg-linear-to-r from-transparent via-indigo-500/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

              <div className="flex items-center gap-2 mb-3">
                <Award size={20} className="text-indigo-400 shrink-0" />
                <span className="text-indigo-400/80 text-xs font-medium">{cert.issuer}</span>
              </div>
              <h3 className="text-white font-semibold mb-2 group-hover:text-indigo-300 transition-colors duration-200">
                {cert.name}
              </h3>
